...
- **目标**：
  ...
  - 支持**单库多 schema** 或**多数据源**：一个 PostgreSQL 库 `pt` 下用 schema `userdb`、`gamedb` 做逻辑隔离时，可只配一个数据源，通过 `@TableName(schema="userdb"|"gamedb", value="表名")` 区分；若将来需要多库，则通过 dynamic-datasource 配置多个数据源，使用 `@DS("xxx")` 切换。
...
- **Mapper XML**（若生成）：`src/main/resources/org/jpstale/dao/userdb/mapper/`、`org/jpstale/dao/gamedb/mapper/`，与包路径对应。
...
- **单库多 schema 示例**：仅配置一个数据源（如 master）指向 `jdbc:postgresql://localhost:5432/pt`，`username`/`password` 使用环境变量 `${DB_USER}`、`${DB_PASSWORD}`；通过实体 `@TableName(schema="userdb"|"gamedb", value="表名")` 区分 schema。
...
- **当前立场**：项目**以 PostgreSQL 为准**，**保持按 schema 隔离**（userdb、gamedb 等），**不计划实际使用 MySQL**。本节内容仅用于**评估若未来要迁到或兼容 MySQL 时的改造难度**，实现时不必优先考虑 MySQL，也无需引入 MySQL 依赖。
...
- 根据**表名 + schema 名**自动生成：
  - 实体类：带 `@TableName(schema="userdb"|"gamedb", value="表名")`，位于 `org.jpstale.dao.<schema>.entity`
...
- 在 pt-dao 中提供可执行类，例如 `org.jpstale.dao.generator.DaoCodeGenerator`：
  ...
  - 对 schema 列表（如 `["userdb", "gamedb"]`）循环执行生成：
...
- **pt-dao 模块**：pom（MyBatis-Plus 注解/API、postgresql、Lombok；可选 mybatis-plus-generator、freemarker）；包结构 `org.jpstale.dao.userdb.entity/mapper`、`gamedb.entity/mapper`、`generator`。
