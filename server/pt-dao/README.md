...
- **位置**：生成器在 **pt-dao 的 src/test/java**（`org.jpstale.dao.generator.DaoCodeGenerator`），生成结果输出到 **本模块**的 `src/main/java` 与 `src/main/resources`。
- **运行**（需本地 pt 库可连，环境变量 `DB_USER` / `DB_PASSWORD` 或默认 postgres/123456）：
  ```bash
  cd server/pt-dao
...
- **生成内容**：各 schema 下的 entity（带 `@TableName(schema="...", value="表名")` 与 Lombok `@Data`）、mapper、service、serviceImpl；不生成 controller。schema 列表在 `DaoCodeGenerator` 中可配置。
